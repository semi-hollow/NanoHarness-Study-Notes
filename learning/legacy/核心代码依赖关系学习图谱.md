...
> 目标：只抓最核心的设计代码和依赖关系，避免从文件树开始盲看。读完后，你应该能用 10～15 个文件讲清楚 Agent Forge 的 single-agent、multi-agent、compare/evaluation 三条主流程。
...
1. `AgentLoop` 是唯一的 canonical runtime。
...
| Runtime loop | `agent_forge/runtime/agent_loop.py` | agent 如何 call model、call tool、接收 observation、结束？ |
...
> Single-agent path 是项目的 canonical runtime。后续 multi-agent 和 benchmark 都尽量复用它，避免出现两套不可比较的执行逻辑。
